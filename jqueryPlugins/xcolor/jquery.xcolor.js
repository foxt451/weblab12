...
(function ($, undefined) {
  ...
  function xColor(color) {
    ...
    this['setColor'] = function (color) {
      ...
      if ('string' === typeof color) {
        ...
        // #ff9000, #ff0000
        if (
          (part = /^#?([0-9a-f]{2})([0-9a-f]{2})([0-9a-f]{2})$/.exec(color))
        ) {
          this['a'] = 1;
        ...
        // rgb(1, 234, 56)
        if (
          (part = /^rgba?\((\d{1,3}),(\d{1,3}),(\d{1,3})(,([0-9.]+))?\)$/.exec(
            color
          ))
        ) {
        ...
        if (
          (part =
            /^hs([bvl])a?\((\d{1,3}),(\d{1,3}),(\d{1,3})(,([0-9.]+))?\)$/.exec(
              color
            ))
        ) {
          let func;
        ...
        // 1, 234, 56
        if (
          (part = /^(\d{1,3}),(\d{1,3}),(\d{1,3})(,([0-9.]+))?$/.exec(color))
        ) {
          this['a'] = _normalize(part[5], 1);
          this['r'] = _normalize(part[1]);
          this['g'] = _normalize(part[2]);
    ...
    this['getName'] = function () {
      if (this.success) {
        ...
        const table = color_names;

        const a = this['getHSL']();

        for (const i in table) {
          /* We do not handle transparency */
          const b = new xColor(table[i])['getHSL']();
  ...
  $(() => {
    ...
    if (
      !($['support']['rgba'] =
        div_style['backgroundColor'].indexOf('rgba') > -1)
    ) {
  ...
  function xColorMix() {
    ...
    this['opacity'] = function (x, y, o) {
      const a = new xColor(x);
      const b = new xColor(y);
    ...
    this['readable'] = function (bg, col, size) {
      ...
      const a = new xColor(col);
      const b = new xColor(bg);
      ...
      if (a.success & b.success) {
        ...
        return !(
          diff < 1.5 + 141.162 * Math.pow(0.975, size) &&
          diff > -0.5 - 154.709 * Math.pow(0.99, size)
        );
      }
      return null;
    };
    ...
    this['breed'] = function (x, y) {
      ...
      if (a.success & b.success) {
        ...
        a['r'] =
          (a['r'] & ((mask >> 0x10) & 0xff)) |
          (b['r'] & (((mask >> 0x10) & 0xff) ^ 0xff));
        a['g'] =
          (a['g'] & ((mask >> 0x08) & 0xff)) |
          (b['g'] & (((mask >> 0x08) & 0xff) ^ 0xff));
        a['b'] =
          (a['b'] & ((mask >> 0x00) & 0xff)) |
          (b['b'] & (((mask >> 0x00) & 0xff) ^ 0xff));
        return a;
      }
      return null;
    };
    ...
    this['gradientarray'] = function (arr, level, deg) {
      ...
      const e = ((level * (arr.length - 1)) / (deg + 1)) | 0;
      const step = deg / (arr.length - 1);

      return $['xcolor']['gradientlevel'](
        arr[e],
        arr[e + 1],
        level - e * step,
        step
    ...
    this['analogous'] = function (col, results, slices) {
      ...
      if (c.success) {
        ...
        for (
          hsv['h'] = (hsv['h'] - ((part * results) >> 1) + 720) % 360;
          --results;
  ...
  $['fn']['readable'] = function () {
    ...
    do {
      ...
      if (
        '' === b &&
        ('transparent' === (b = $['css'](elem, 'backgroundColor')) ||
          'rgba(0, 0, 0, 0)' === b)
      ) {
        b = '';
      }
  ...
  $['fn']['colorize'] = function (FROM, TO, TYPE) {
    ...
    this['each'](function () {
      let tmp = this.childNodes,
        LEN = 0,
        K = 0;

      if (FROM.success & TO.success) {
        for (let i = tmp.length; i--; LEN += tmp[i]['textContent'].length) {}
